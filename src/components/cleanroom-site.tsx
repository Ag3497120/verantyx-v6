...
export default function CleanroomSite({base='',marketing=false,author=false,themeControl}:{base?:string;marketing?:boolean;author?:boolean;themeControl?:ReactNode}){
  ...
  return <div className="cr-site" lang={locale}>
    ...
    <main id="main">
      ...
      <section className="cr-origin"><span className="cr-index">04 / WHY I STARTED</span><h2>{ja?'作る経験まで、手放したくなかった。':'I did not want to lose the experience of making.'}</h2><p>{l.origin}</p><div><Link href={authorLink}>{l.author} ↗</Link><a href={REPO+'/blob/main/core/docs/origins/README.md'}>{ja?'原文と開発記':'Sources & development notes'} ↗</a></div></section>
    ...
    <footer className="cr-site-footer"><Link className="cr-small-brand" href={base+'/'}><Image unoptimized src={asset('mark.svg')} alt="" width="32" height="32"/>cleanroom</Link><p>{l.foot}</p><Link href={authorLink}>{l.author}</Link><a href={REPO}>GitHub</a>{marketing&&<Link href="/apps/">Other projects</Link>}</footer>
  </div>;
}

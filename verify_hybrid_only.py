#!/usr/bin/env python3
"""Verify hybrid eval results only (DeepSeek + Verantyx cross engine)."""
import json, os

EVAL_DIR = "/private/tmp/arc-agi-2/data/evaluation"

DIRS = {
    "Cross Engine": ("hybrid_cross_results", "json"),
    "DS+Cross Synth": ("hybrid_synth_results", "py"),
    "DS Direct": ("hybrid_direct_results", "json"),
}
...
for label, (dirname, ext) in DIRS.items():
    ...
    if passed or failed:
        print(f"{label}: {len(passed)} PASS / {len(passed)+len(failed)} total")
        for t in passed: print(f"  ✅ {t}")
        all_passed.update(passed)

print(f"\n{'='*50}")
print(f"Hybrid (DeepSeek + Verantyx): {len(all_passed)}/120 ({100*len(all_passed)/120:.1f}%)")
